...
def load_data_with_missing(url: str | None=None, file_path: Path | None=None) -> pd.DataFrame:
    ...
    if file_path and Path(file_path).exists():
        ...
        passengers = 100 + 20 * np.sin(np.arange(144) / 12) + np.random.normal(0, 10, 144)
...
def analyze_missing_data(df: pd.DataFrame) -> dict:
    ...
    return {'missing_counts': df.isnull().sum().to_dict(), 'missing_percentages': (df.isnull().sum() / len(df) * 100).to_dict(), 'total_missing': df.isnull().sum().sum()}
...
def plot_missing_analysis(df: pd.DataFrame, title: str, output_path: Path, plot: bool=False):
    ...
    if missing_counts.sum() > 0:
        ax.bar(range(len(missing_counts)), missing_counts.values, color='#4A90A4', alpha=0.7, edgecolor='none')
        ax.set_xticks(range(len(missing_counts)))
        ax.set_xticklabels(missing_counts.index, rotation=45, ha='right')
    else:
        ax.text(0.5, 0.5, 'No missing values', ha='center', va='center', transform=ax.transAxes, fontsize=14)
